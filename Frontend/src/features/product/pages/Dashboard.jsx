    ...
    <div className="min-h-screen bg-surface font-body-md text-on-surface">
        ...
        <div className="max-w-7xl mx-auto px-6 py-4 flex justify-between items-center">
          ...
          <div className="flex items-center space-x-6">
            <div className="hidden md:block text-right">
              <p className="text-xs font-semibold text-on-surface-variant uppercase tracking-widest">Logged in as</p>
              <p className="text-sm font-playfair italic text-primary">{user?.fullname || "Artisan"}</p>
            </div>
            <button 
      ...
      <main className="max-w-7xl mx-auto px-6 py-10 space-y-12">
        ...
        <section className="relative overflow-hidden rounded-3xl bg-primary-container/10 p-8 md:p-12 border border-primary/5">
          ...
          <div className="relative z-10 space-y-4">
            <h2 className="text-4xl md:text-5xl font-playfair text-primary">
              Welcome to your Atelier, <span className="italic">{user?.fullname?.split(" ")[0]}</span>
            </h2>
            <p className="text-on-surface-variant font-light text-lg max-w-2xl leading-relaxed">
              Your digital space to showcase the heritage of your craft. Manage your masterpieces, track their journey, and connect with collectors of fine textiles.
            </p>
          </div>
        </section>
        ...
        <section className="grid grid-cols-1 md:grid-cols-2 gap-6 max-w-4xl mx-auto">
          {[
            { label: "Total Creations", value: products?.length || 0, icon: "inventory_2", color: "text-primary" },
            { label: "Active Listings", value: products?.length || 0, icon: "check_circle", color: "text-secondary" },
          ].map((stat, i) => (
            <div key={i} className="bg-white p-8 rounded-2xl border border-outline-variant/20 shadow-sm hover:shadow-md transition-shadow group">
              <div className="flex items-center justify-between mb-4">
                <span className={`material-symbols-outlined ${stat.color} text-3xl opacity-80 group-hover:scale-110 transition-transform`}>
                  {stat.icon}
                </span>
                <span className="text-4xl font-playfair text-on-surface">{stat.value}</span>
              </div>
        ...
        <section className="space-y-8">
          <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 border-b border-outline-variant/20 pb-6">
            <div className="space-y-2">
              <h3 className="text-3xl font-playfair text-primary">Your Masterpieces</h3>
              <p className="text-on-surface-variant font-light">A curated collection of your heritage textiles.</p>
            </div>
            <div className="flex items-center space-x-4">
          ...
          {products?.length === 0 ? (
            <div className="py-24 text-center space-y-6 bg-white rounded-3xl border-2 border-dashed border-outline-variant/30">
              ...
              <div className="space-y-2">
                <h4 className="text-2xl font-playfair text-on-surface">No creations found in your atelier</h4>
                <p className="text-on-surface-variant font-light">Begin your journey by adding your first heritage piece.</p>
              </div>
              <button 
                onClick={() => navigate("/seller/create-product")}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
              {products.map((product) => (
                <div key={product._id} className="group bg-white rounded-2xl overflow-hidden border border-outline-variant/10 shadow-sm hover:shadow-xl transition-all duration-500">
                  <div className="relative aspect-[4/5] overflow-hidden">
                    <img 
                      src={product.images?.[0]?.url || "https://images.unsplash.com/photo-1621419777150-136195c807b5?q=80&w=600&auto=format&fit=crop"} 
                      alt={product.title}
                  ...
                  <div className="p-6 space-y-4">
                    <div className="space-y-1">
                      <p className="text-[10px] text-secondary font-bold uppercase tracking-widest">Heritage Weave</p>
                    ...
                    <div className="pt-4 flex items-center justify-between border-t border-outline-variant/10">
                      ...
                      <div className="flex items-center space-x-3">
                        <button className="material-symbols-outlined text-outline hover:text-primary transition-colors text-lg">
                          edit
                        </button>
                        <button
      ...
      <footer className="py-20 flex flex-col items-center opacity-20 select-none">
        <p className="text-[10px] tracking-[0.5em] font-light text-on-surface uppercase mb-4">Vastraa Heirloom Traditions</p>
        <div className="flex items-center space-x-4">
